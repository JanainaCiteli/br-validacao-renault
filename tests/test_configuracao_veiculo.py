    ...
    if _is_versoes_page(ctx):
        ...
        if total_btns > idx:
            ...
            try:
                ...
                if el:
                    ctx.evaluate("(el) => el.scrollIntoView({behavior:'auto', block:'center'})", el)
            except Exception:
                pass
...
# ==== Diagnóstico de modelo/versão + rede/console ====

def _modelo_versao_atual(page, ctx):
    """
    Retorna (modelo, versao) com base em URL e elementos visíveis.
    """
    modelo = None
    versao = None
    ...
    # Versão por combobox selecionado
    try:
        combo = ctx.get_by_role("combobox", name=re.compile(r"Vers[aã]o", re.I)).first
        if combo and combo.count() > 0:
            el = combo.element_handle()
    ...
    # Versão por candidatos visíveis
    if not versao:
        candidatos = [
            ctx.get_by_role("heading").first,
            ctx.locator('[data-testid*="version"], [class*="version"], [class*="versao" i]').first,
            ctx.get_by_text(re.compile(r"Iconic|Intense|Zen|Outsider|E-Tech", re.I)).first,
        ]
        for c in candidatos:
            try:
                if c and c.count() > 0 and c.is_visible():
    ...
    return (modelo or "DESCONHECIDO", versao or "DESCONHECIDA")


def _instalar_coletores_diagnostico(page):
    erros_console = []
    respostas_ruins = []  # (status, url)
    ...
    def on_response(resp):
        try:
            url = resp.url or ""
            status = resp.status
            if ("/configurador/" in url or "3dv.renault.com" in url) and status >= 400:
                respostas_ruins.append((status, url))
        except Exception:
            pass
    ...
    return erros_console, respostas_ruins


def _diagnosticar_sem_opcoes(page, ctx, etapa: str, modelo: str, versao: str, erros_console, resp_ruins):
    """
    Retorna string com motivo provável para ausência de opções.
    """
    # 1) Mensagens na UI
    try:
        msg = ctx.get_by_text(re.compile(r"indispon[ií]vel|aguarde|carregando|n[oã]o h[aá]|sem op[cç][oõ]es", re.I)).first
        if msg and msg.count() > 0 and msg.is_visible():
            return f"UI indica indisponibilidade: '{msg.inner_text().strip()[:140]}'"
    except Exception:
        pass

    # 2) CTA 'Avançar' desabilitado (pré-requisito pendente)
    try:
        btn = ctx.get_by_role("button", name=re.compile(r"Avan[cç]ar|Continuar|Concession[aá]ria", re.I)).first
        if btn and btn.count() > 0:
            el = btn.element_handle()
            disabled = (el.get_attribute("disabled") if el else None)
            aria_dis = (el.get_attribute("aria-disabled") if el else None)
            if disabled or (aria_dis and aria_dis.lower() == "true"):
                return "CTA de avanço desabilitado — pode haver pré-requisito/seleção pendente."
    except Exception:
        pass

    # 3) Respostas 4xx/5xx da API/3D
    if resp_ruins:
        for st, url in resp_ruins:
            if "3dv.renault.com" in url:
                return f"Falha no render 3D (status {st}): {url}"
        st, url = resp_ruins[0]
        return f"Resposta de rede com erro (status {st}): {url}"

    # 4) Erros no console
    if erros_console:
        return f"Erros de console: {erros_console[:2]}"

    # 5) Fallback: versão sem personalização (pré-configurada)
    return "Possível versão pré-configurada (sem personalização para etapa atual)."


def _selecionar_todas_opcoes_com_diagnostico(page, ctx, request, erros_console, resp_ruins, tipo: str, contexto: str, limit: int = 0):
    """
    Wrapper que diagnostica ausência de opções e registra no relatório HTML.
    """
    _ir_para_etapa(ctx, tipo)
    loc = _coletar_opcoes(ctx, tipo)
    total = 0
    ...
    modelo_nome, versao_nome = _modelo_versao_atual(page, ctx)

    if total == 0:
        motivo = _diagnosticar_sem_opcoes(page, ctx, tipo, modelo_nome, versao_nome, erros_console, resp_ruins)
        aviso = f"[AVISO] Sem opções para '{tipo}' - Modelo {modelo_nome} | Versão {versao_nome} | Motivo: {motivo}"
        print(aviso)
        _anexar_screenshot(request, page, aviso)
        return

    # Se há opções, delega para a função principal
    _selecionar_todas_opcoes(ctx, tipo, contexto, limit)


def _abrir_concessionaria_via_stepper(ctx, page: Page):
    """
    Fallback: clica explicitamente na aba/link 'Concessionária' do stepper e aguarda o conteúdo carregar.
    """
    # Tenta tab
    try:
    ...
    # Tenta link/click por texto
    for role in ("link", "button"):
        try:
            al = ctx.get_by_role(role, name=re.compile(r"Concession[aá]ria", re.I)).first
            if al and al.count() > 0 and al.is_visible():
                el = al.element_handle()
                if el:
                    ctx.evaluate("(el) => el.scrollIntoView({behavior:'auto', block:'center'})", el)
                al.click(timeout=10000)
                try:
                    page.wait_for_load_state("networkidle", timeout=4000)
                except Exception:
                    pass
...
def _goto_concessionaria_por_url(page: Page):
    """
    Constrói a URL de concessionária a partir da URL atual do configurador e navega até ela.
    Ex.: /configurador/kwid/design -> /configurador/kwid/concessionaria
    """
    url = page.url
    ...
    if not m:
        return False
    modelo = m.group(1)
    destino = f"https://loja.renault.com.br/configurador/{modelo}/concessionaria/"
    try:
        page.goto(destino, wait_until="domcontentloaded", timeout=35000)
        try:
            page.wait_for_load_state("networkidle", timeout=4000)
        except Exception:
            pass
...
def _esperar_concessionaria(page: Page, ctx, timeout_ms: int = 30000) -> bool:
    """
    Espera chegada à Concessionária:
    - page.url ou frame.url (regex)
    - OU conteúdo no ctx: heading 'Concessionária', inputs de CEP, lista de concessionárias, botões 'Selecionar'
    """
    ...
    # 2) Tentativa por URL nos frames
    ...
    # 3) Checagem por conteúdo no ctx
    try:
        head = ctx.get_by_role("heading", name=re.compile(r"Concession[aá]ria", re.I)).first
    ...
    try:
        ...
        if tab and tab.count() > 0 and tab.is_visible():
            el = tab.element_handle()
            sel = el.get_attribute("aria-selected") if el else None
    ...
    sinais = [
        ctx.locator('input[placeholder*="CEP" i]'),
        ctx.get_by_role("button", name=re.compile(r"Selecionar\s+concession[aá]ria", re.I)),
        ctx.locator('[data-testid*="dealer"], [data-testid*="dealers"], [class*="dealer"]'),
    ]
    try:
        for s in sinais:
            if s and s.count() > 0 and s.first.is_visible():
                return True
    except Exception:
        pass
...
def test_configuracao_veiculo_para_todos_modelos_e_versoes(page: Page, request):
    ...
    erros_console, resp_ruins = _instalar_coletores_diagnostico(page)
        ...
        try:
            ...
            for v_idx in range(versoes_iter):
                try:
                    ...
                    contexto_base = f"Modelo {m_idx} | Versão {v_idx}"

                    # Cores (todas)
                    _selecionar_todas_opcoes_com_diagnostico(page, ctx, request, erros_console, resp_ruins, "cor", contexto_base, CORES_LIMIT)

                    # Rodas (todas)
                    _selecionar_todas_opcoes_com_diagnostico(page, ctx, request, erros_console, resp_ruins, "rodas", contexto_base, RODAS_LIMIT)

                    # Interior (todas)
                    _selecionar_todas_opcoes_com_diagnostico(page, ctx, request, erros_console, resp_ruins, "interior", contexto_base, INTERIOR_LIMIT)

                    # Avançar para Concessionária
                    _clicar_avancar(ctx, f"Modelo {m_idx} | Versão {v_idx} | Final")

                    # 1) URL/ctx
                    if not _esperar_concessionaria(page, ctx, 35000):
                        # 2) Stepper/tab
                        abriu = _abrir_concessionaria_via_stepper(ctx, page)
                        if not abriu or not _esperar_concessionaria(page, ctx, 25000):
                            # 3) Navegação direta
                            foi = _goto_concessionaria_por_url(page)
                            if not foi or not _esperar_concessionaria(page, ctx, 25000):
                    ...
                    # Enriquecer mensagem com nome do modelo/versão
                    modelo_nome, versao_nome = _modelo_versao_atual(page, ctx)
                    erros.append(f"[{modelo_nome} | {versao_nome}] {str(e)}")
