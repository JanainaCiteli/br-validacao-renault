...
def pytest_runtest_makereport(item, call):
    ...
    try:
        ...
        if rep.outcome == "skipped":
            # contabiliza
            count = getattr(config, "_skip_count", 0)
            setattr(config, "_skip_count", count + 1)

            # Modo estrito: falha se FAIL_ON_SKIP=true
            if os.getenv("FAIL_ON_SKIP", "").lower() in ("1", "true", "yes", "on"):
                rep.outcome = "failed"
                rep.longrepr = "Skip convertido em falha (FAIL_ON_SKIP habilitado)."
    except Exception:
        pass
...
def pytest_html_results_summary(prefix, summary, postfix):
    try:
        ...
        config = None
    #novo    
    skip_count = getattr(config, "_skip_count", 0) if config else 0
    if skip_count > 0:
        prefix.extend([pytest_html.extras.html(f"<p>⚠️ Skips detectados: {skip_count}. Reveja os testes ou ative FAIL_ON_SKIP para falhar o pipeline.</p>")])
